...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between items-center w-full h-20 bg-white fixed">
        <div>
          <h1 className="text-5xl font-signature ml-2">K and S</h1> 
          <h2  className="ml-8 font-bold">Mobiled Mechanic</h2> 
        </div>
            <ul className="hidden md:flex">
                {links.map(({id,link}) => (
                    <li 
                    key={id} 
                    className="px-4 cursor-pointer capitalize font-medium    text-2xl hover:scale-110 duration-200">
            ...
            {nav ? <FaTimes size={30} className="text-gray-60"/> :  <FaBars size={30} className="hover:scale-105 duration-20" /> }
            </div>
